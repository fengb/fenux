...
pub const Errno = enum(u12) {
    pub const E = blk: {
        const fields = std.meta.fields(Errno);
        var errors: [fields.len]std.builtin.TypeInfo.Error = undefined;
        for (fields) |field, i| {
    ...
    pub fn from(err: E) Errno {
        inline for (std.meta.fields(Errno)) |field| {
            if (err == @field(E, field.name)) {
                return @field(Errno, field.name);
            }
        }
        unreachable;
    }
